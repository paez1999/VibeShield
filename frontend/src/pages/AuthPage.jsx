...
const s = {
  ...
  logoText: { fontFamily: 'var(--font-disp)', fontWeight: 800, fontSize: '20px', color: '#fff' },
  tabs: { display: 'flex', marginBottom: '28px', borderBottom: '1px solid var(--border)' },
  tab: (active) => ({
    flex: 1, padding: '10px', textAlign: 'center', fontSize: '12px',
    fontFamily: 'var(--font-mono)', background: 'transparent', border: 'none',
    color: active ? 'var(--accent)' : 'var(--muted)',
    borderBottom: active ? '2px solid var(--accent)' : '2px solid transparent',
    marginBottom: '-1px', transition: 'all .15s', letterSpacing: '1px', cursor: 'pointer',
  }),
  ...
  input: {
    ...
    fontFamily: 'var(--font-mono)', fontSize: '13px', outline: 'none', transition: 'border-color .15s',
  },
  btn: (loading) => ({
    width: '100%', marginTop: '8px', padding: '12px',
    background: loading ? 'var(--accent-dim)' : 'var(--accent)',
    border: 'none', borderRadius: 'var(--radius)', color: '#0a0c0f',
    fontFamily: 'var(--font-disp)', fontWeight: 700, fontSize: '14px',
  ...
  hint: { textAlign: 'center', color: 'var(--muted)', fontSize: '11px', marginTop: '24px' },
}
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode === 'login') {
        ...
        await signup(form.orgName, form.email, form.password)
      }
      navigate('/dashboard')
    } catch (err) {
  ...
  return (
    <div style={s.page}>
      <div style={s.card}>
        ...
        <form onSubmit={handleSubmit}>
          {mode === 'signup' && (
            <div style={s.field}>
              ...
              <input style={s.input} placeholder="Acme Music Inc"
          ...
          <div style={s.field}>
            ...
            <input style={s.input} type="email" placeholder="admin@yourapp.com"
        ...
        <p style={s.hint}>
          {mode === 'login' ? 'No account? Click Sign Up above' : 'Already registered? Click Login above'}
        </p>
      </div>
    </div>
  )
